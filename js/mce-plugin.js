(function() {
	...
	/*
	 * Variables to save the instances of the button and the editor for use in
	 * the callbacks, plus one for jquery.
	 */
	var iliButton = null;
	var theEditor = null;
	var $ = jQuery;
	...
	tinymce.create('tinymce.plugins.ili_mce_plugin', {
		init : function(ed, url) {
			...
			ed.addCommand('indent_lists', function() {
				if (e = theEditor.selection.getNode()) {
					/* Indent a list if present in this main node */
					$(e).closest('ul, ol').toggleClass('ili-indent');
			...
			ed.on('NodeChange', function() {
				if (iliButton) {
					...
					list = $(theEditor.selection.getNode()).closest('ul, ol');
